...
We obtained this data from a Tidy Tuesday dataset and it was originally scraped from www.sports-reference.com. The data set contains data from the Olympics from 1896 to 2016. Each athlete's name is listed and they have been assigned an individual ID. Physical characteristics are comprised of their sex, age, height, and weight. Team and National Olympic Committee Region (NOC) are used to denote which region the athlete represents. However, an observed anomaly was for each nation of interest there were multiple team names over the years and as a result NOC best represented each country of interest. Additionally, the dataset contained information about the Olympic Games the athlete competed in, including the year, the season (summer or winter), and the city the games were held in. The sport and event were also included and, of course, which medal the athlete won (gold, silver, bronze), if an athlete did not medal the medal variable was given an NA. No event specific information was included (i.e. time or score).
...
![Figrue 1: All 230 nations in the original dataset. Figure generated with Package Wordclouds2.](#allofthenations.png) ![Figure 2: All 1184 teams in the original data set.Figure generated with Package Wordclouds2.](#alloftheteams.png)
...
### Historical Conflict & the Olympics
...
#| out-width: 80% #| fig-cap: "This figure shows the influence of global conflict on Olympic game participation in males and females from 1896 to 2016. The figure is drawn with ggplot2 [@ggplot2-package]."
...
Several figures were created to look at the changes of track events over time. @fig-track_events is a scatter plot that shows a point for each year the event was hosted at the Olympic Games. The points were color coded so we can see if the changes affects one category more than another. @fig-track_events was generated with `ggplot2` [@ggplot2-package]. @fig-multi_events was also created to show the distribution of the number of events each individual athlete competed, and how this changes over time.
...
Inital view of selection page ![selection](mainopeningpage.png)

Math for us dummies ![math](Mathopening%20page.png)
...
Obervations from application - At the age of 37 Nicole would have not have had a counter part
